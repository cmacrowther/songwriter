...
export default class SpotifyData extends Component {
  ...
  getData() {
    if (!this.props.title) {
      ...
      const description =
        "This error can be rectified by accessing the admin panel and updating your 'Site Settings'"
      ...
      GetToken().then((credentials) =>
        setTimeout(() => {
          if (credentials.access_token) {
            ...
            const preamble =
              'Error encountered while generating Spotify Token: '
            this.setState({
              error: preamble + credentials.error,
  ...
  render() {
    return this.state.token && !this.props.isCms
      ? renderSongData(this.props, this.state.token)
      : renderSkeleton(this.state.error, this.state.description)
  }
}
